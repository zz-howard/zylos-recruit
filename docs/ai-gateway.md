...
每次调用时，Gateway 通过 `resolve(scenario)` 解析运行时配置：
...
|------|------|-------------|
| `text` | 文本生成 | 全部 |
| `read_file` | 读取本地文件 | claude, codex, gemini |
...
`detectRuntimes()` 遍历所有注册适配器，调用 `isAvailable()` 检查 CLI 是否存在（通过 `which` 命令）或 token 是否存在（HTTP 运行时），返回当前系统可用的运行时列表。
...
5. **（可选）集成沙箱**：如果 CLI 适配器需要文件系统隔离，引入 SRT 层（见 SRT 文档）
