...
- **deny HOME/project root + allowlist**：默认保护用户主目录和项目敏感目录，按需精确放回运行时/认证/场景路径
...
SRT 提供域名级网络白名单（通过 HTTP/SOCKS 代理 + `--unshare-net`）。这是 allow-only 模型，不是 blacklist 模型：
...
当前 zylos-recruit 默认未启用网络隔离（`network: {}`），因为面试场景需要 WebFetch 访问候选人任意 URL，而 SRT 当前没有“允许任意公网但拒绝 localhost/内网”的 deny-only 模式。安全主要由文件系统隔离兜底——即使 AI 能访问网络，也无法读取到敏感文件用于外泄。

如果在 `ai.sandbox.network.allowedDomains` 或代码层 `sandbox.network.allowedDomains` 中显式配置 allowlist，Recruit 会启用 SRT 网络隔离，并自动附加 allowlist 内的优先排除项（priority-deny entries within allowlist，例如 `localhost`、`127.0.0.1`、`169.254.169.254`、`metadata.google.internal` 等）。这些 `deniedDomains` 只有在 `allowedDomains` 存在、SRT 代理启用后才参与判定；它们不是独立黑名单。注意：这会限制 WebFetch 只能访问 allowlist 中且未被优先排除的域名，不适合作为 Recruit 默认策略。

网络配置的优先级是：代码层 `sandbox.network` > 场景级 `ai.<scenario>.sandbox.network` > 全局 `ai.sandbox.network`。场景级配置不是与全局配置合并，而是替换全局网络策略；因此每个场景可以缩小 allowlist，也可以设置 `network: {}` 显式保持 unrestricted。如果只想限制某个场景，把 allowlist 放在该场景配置里，不要放在 global 层。
...
```javascript
{
  ...
  network: {                 // 网络策略（可选）
    allowedDomains: string[], // SRT allow-only；存在时启用网络隔离
    deniedDomains: string[],  // allowlist 内的优先排除项；不能单独构成 deny-only 策略
    // 场景级 network 替换全局 network；network: {} 可显式 opt out
  },
}
```
...
```javascript
{
  network: {},                    // 默认空对象 = 不隔离网络，保留任意 WebFetch
  filesystem: {
...
**结论**：SRT 配置中 `network: {}` 表示不配置网络策略（不启用 `--unshare-net`），进程可正常访问网络。只有 `network.allowedDomains` 存在时才触发网络隔离。`allowedDomains` 是严格白名单；`deniedDomains` 是该白名单内的优先排除项，判定顺序是先查 `deniedDomains`，再查 `allowedDomains`，都不命中则拒绝。没有 `allowedDomains` 时，`deniedDomains` 不会作为独立黑名单生效。

Recruit 的配置解析把全局 `ai.sandbox.network` 当作默认值；如果场景级 `ai.<scenario>.sandbox.network` 存在，它会替换全局网络策略，而不是合并。这样场景可以自行选择更窄、更宽或 `network: {}` unrestricted，不会被 global allowlist 强制套住。
