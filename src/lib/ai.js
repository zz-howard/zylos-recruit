...
export async function rankRolesFromResume(candidateId) {
  ...
  const prompt = `你是一位资深招聘专家，擅长深度理解岗位本质和候选人能力的真实匹配度。
...
1. **理解岗位定位**：这个岗位到底要什么样的人？核心职责是什么？不要被岗位名称误导，要看画像/JD 的实质要求。

2. **解读画像结构**：如果岗位画像包含分层信息（必备条件、加分项、减分项/红线），严格按层级处理：
   - **必备条件**：候选人必须满足大部分，否则分数上限 50
   - **加分项**：在必备条件满足的基础上加分
   - **红线/减分项/明确排除条件**：一旦触发任何一条红线，该岗位得分上限 30，无论其他方面多匹配

3. **区分匹配维度**：
   - **领域匹配**：候选人的经验领域是否与岗位相关（如：都是 NLP 方向 vs 候选人做 RAG 但岗位要模型训练）
   - **深度匹配**：候选人在该领域的深度/资历是否达到岗位要求
   - **仅有表面关键词重叠但实质不匹配的，不应获得高分**

4. **评分标准**：
   - 90-100：核心条件高度匹配，无红线，有明显加分项
   - 70-89：核心条件基本匹配，无红线，可能缺少部分加分项
   - 50-69：部分匹配但有明显短板，或核心条件只满足一半
   - 30-49：匹配度低，主要靠边缘关联
   - 0-29：触发红线，或完全不相关

请对**每一个**岗位都给出评估结果，按匹配度从高到低排序，以 JSON 格式输出：
{"matches": [{"role_id": <数字>, "role_name": "<岗位名称>", "score": <0-100 整数>, "reason": "<简要说明匹配/不匹配的核心原因，如触发红线需指出具体哪条>"}]}
